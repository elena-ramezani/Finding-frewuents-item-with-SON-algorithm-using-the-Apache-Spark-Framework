...
def count_candidate_pair(baskets, pairs, threshold):
    ...
    for basket in baskets:
        for pair in pairs:
            ...
            newpair = set(pair)
            newpair_sort = sorted(newpair)
            pair_tuple = tuple(newpair_sort)
            if newpair.issubset(basket):
                freq_item[pair_tuple] = freq_item.get(pair_tuple,0)+1
...
def apriori(chunk, support, total_num):
    ...
    final_frequent_item.extend(final_pair)
    #print(final_frequent_item)
    length_pair = 3
    ...
    while len(freq_item) != 0:
        ...
        final_frequent_item.extend(freq_item)
        freq_item.sort()
...
if __name__ == '__main__':
    ...
    parser.add_argument('threshold', nargs='+', help='threshold')
    ...
    support_number = args.support
    input_file_name = args.input_file
    output_file_name = args.output_file
  

    filter_threshold = int(threshold[0])
    support = int(support_number[0])
    ...
    data = rdd_csv.filter(lambda x: x != header)

    mapped_rdd_by_busid = data.map(lambda x: x.split(",")).map(lambda x: (x[0], x[1])).groupByKey()

    mapped_rdd_by_busid1 = data.map(lambda x: x.split(",")).map(lambda x: (x[0], 1)).groupByKey().mapValues(sum)

    user = mapped_rdd_by_busid1.filter(lambda x: x[1] > filter_threshold)

    baskets = user.join(mapped_rdd_by_busid).map(lambda x: x[1]).map(lambda x: set(x[1]))
    num = baskets.count()

    output_phase_one = baskets.mapPartitions(lambda basket: apriori(basket, support, num)).map(lambda x: (x, 1))
    output = output_phase_one.reduceByKey(lambda x, y: (1)).keys().collect()

    output_phase_two = baskets.mapPartitions(lambda baskets: candidateCount(baskets, output))
